...
public class RincianDataPembelian extends javax.swing.JFrame {
    ...
    public void setData(int id) {
        ...
        String query = "select tanggal_beli, id_supplier, id_barang, kuantitas, total_harga, status from pembelian where id = ?";
        
        try {
            PreparedStatement stat = conn.prepareStatement(query);
            stat.setInt(1, idPembelian);
            ResultSet hasil = stat.executeQuery();
            
            while(hasil.next()) {
                tanggal_beli.setDate(hasil.getDate(1));
                cbnamasupplier.setSelectedIndex(hasil.getInt(2) - 1);
                cbnamabarang.setSelectedIndex(hasil.getInt(3) - 1);
                kuantitas.setText(hasil.getString(4));
                transaksi.setText(hasil.getString(5));
                
                if(hasil.getString(6).equals("Lunas")) {
    ...
    private void editButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_editButtonActionPerformed
        ...
        try {
            String sql = "update pembelian set tanggal_beli=?, id_supplier=?, id_barang=?, kuantitas=?, total_harga=?, status=? where id=?";
            PreparedStatement stat = conn.prepareStatement(sql);
            stat.setDate(1, new java.sql.Date(tanggal_beli.getDate().getTime()));
            stat.setInt(2, cbnamasupplier.getSelectedIndex() + 1);
            stat.setInt(3, cbnamabarang.getSelectedIndex() + 1);
            stat.setInt(4, Integer.parseInt(kuantitas.getText()));
            stat.setFloat(5, Float.parseFloat(transaksi.getText()));
            stat.setString(6, status.getSelectedItem().toString());
            stat.setInt(7, idPembelian);
            stat.executeUpdate();
